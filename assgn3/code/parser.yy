...
IDENTIFIER '(' ')' ';'
{
   ...
   if ($1 == "printf" || $1 == "scanf") {
      error(@$, string("printf needs atleast one string parameter"));
   }
   else {
...
| IDENTIFIER '(' expression_list ')' ';'
{
   ...
   if (func_params.find($1) == func_params.end() && $1!="printf" && $1!="scanf")
      error(@$, string("function with name ") + $1 + string(" not defined"));
   if ($1 == "printf" || $1 == "scanf") {
      if (args[1]->ett.print()!=string("char*"))
         error(@$, string("printf requires first argument to be string"));
      proccall_astnode* node = new proccall_astnode(args);
      $$ = node;
   }
   else{
...
| IDENTIFIER '(' expression_list ')'
{
   ...
   if (globname==$1) {
      node->ett.strcnt = 0;
      node->ett.array_sz = vector<int>();
      node->ett.sec_strcnt = 0;
      node->ett.lval = false;
      node->ett.basetype = globtypespec;
      $$ = node;
   }
   else {
      typeTree tt = gst.Entries[$1].type;
...
| STRING_LITERAL
{
   ...
   node->ett.da_lc = lcvec.size();
   LC temp;
   temp.da_str = $1;
   temp.da_func = globname;
   lcvec.push_back(temp);
   $$ = node;
}
| '(' expression ')'
{
